...
mod prelude {
    ...
    pub use piet::RenderContext;
    pub use piet_cairo::CairoRenderContext;
...
pub fn render_to_image(
    ...
    let mut cr = cairo::Context::new(&surface);
    ...
    render_to_canvas(tree, opt, img_view, &mut cr);
...
pub fn render_node_to_image(
    ...
    render_node_to_canvas(node, opt, vbox, img_size, &mut cr);
...
fn render_group_impl(
    ...
    let bbox = {
        let mut sub_cr = cairo::Context::new(&*sub_surface);
        sub_cr.set_matrix(curr_ts);

        render_group(node, opt, layers, &mut sub_cr)
    };
